...
The dialog field order is **Stock**, **Quantity**, then the **Buy price**/**Buy date** pair, then the **Sell price**/**Sell date** pair, and finally **Note** at the bottom. Stock and at least one price are required; Quantity, both dates, and Note may be left empty. A price may be recorded without its matching date, but a date cannot be recorded without its matching price. The stock combobox has the placeholder **Select or search a stock**. When a live quote is available for the selected stock, its current price and percentage are shown inside the combobox after the stock name, using the same red/green market-change convention as the table. Clicking its arrow opens the locally recorded stocks. Typing a name or code and pressing Enter replaces that list with matching `stock-api` results. Clearing the input and pressing Enter restores the locally recorded-stock list. Clicking anywhere outside the combobox closes its open list.
